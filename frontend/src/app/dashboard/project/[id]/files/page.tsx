...
// === Types that match your directory schema ===
type FileMeta = {
  key: string;
  name: string;
  size: string; // e.g. "0.1MB"
  uploadedAt: string; // YYYY-MM-DD
  path?: string[]; // optional trail
};
...
type DirectoryRoot = DirectoryFolder;
...
export default function FilesPage() {
  ...
  // Immutable project.directory updater
  const updateDirectory = useCallback(
    (updater: (dir: DirectoryRoot) => void) => {
      setProject((prev) => {
        const updated = structuredClone(prev);
        if (updated != null) {
            updater(updated.directory);
        }
        return updated;
      });
    },
  ...
  // Find directory at currentPath
  const getCurrentDirectory = useCallback((): DirectoryFolder | null => {
    let current: DirectoryFolder = project.directory;
    ...
    return current;
  }, [project.directory, currentPath]);

  const currentDir = useMemo(getCurrentDirectory, [getCurrentDirectory]);
  ...
  // Presign GET for preview
  const getFileUrl = useCallback(
    async (file: FileMeta): Promise<string> => {
      const res = await fetch(
        `http://localhost:5000/api/project/${project.projectId}/files/presign-get`,
        {
          method: "POST",
          credentials: "include",
      ...
      if (!res.ok) {
        throw new Error("Failed to presign GET");
      }
  ...
  // Preview effect (with proper deps)
  useEffect(() => {
    let cancelled = false;
  ...
  const uploadFile = useCallback(
    async (file: File): Promise<void> => {
      const presignRes = await fetch(
        `http://localhost:5000/api/project/${project.projectId}/files/presign`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
      ...
      const metaRes = await fetch(
        `http://localhost:5000/api/project/${project.projectId}/files/metadata`,
        {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
      ...
      // Update UI
      updateDirectory((dir) => {
        let dirRef: DirectoryFolder = dir;
        for (const name of currentPath) {
    ...
    [currentPath, project.projectId, updateDirectory]
  );
  ...
  const handleFolderCreate = useCallback(async () => {
    ...
    const { folder }: { folder: DirectoryFolder } = await res.json();
    ...
    setNewFolderName("");
  }, [currentDir, currentPath, newFolderName, project.projectId, updateDirectory]);
  ...
  const handleFileDelete = useCallback(
    async (index: number) => {
      ...
      const res = await fetch(
        `http://localhost:5000/api/project/${project.projectId}/files`,
        {
          method: "DELETE",
          headers: { "Content-Type": "application/json" },
          credentials: "include",
    ...
    [currentDir, currentPath, previewFile?.key, project.projectId, updateDirectory]
  );
  ...
  const handleFolderDelete = useCallback(
    ...
    [currentDir, currentPath, project.projectId, updateDirectory]
  );
    ...
    <div className="p-6 pt-24 md:pl-72 bg-[#f9fafa] min-h-screen flex flex-col">
      ...
      <div className="flex justify-between items-center mb-4">
        ...
        <div className="flex gap-2">
          ...
          <label className="bg-gray-100 border border-gray-300 px-3 py-1 rounded-lg text-sm flex items-center gap-1 cursor-pointer hover:bg-gray-200 transition">
            <input
              type="file"
              multiple
      ...
      <div className="flex gap-6 flex-1">
        {/* Main content */}
        <div
          className={`flex-1 border border-gray-200 rounded-lg overflow-hidden transition ${
            dragOver ? "ring-2 ring-blue-400" : "bg-white"
          }`}
          onDragOver={(e) => {
            e.preventDefault();
            setDragOver(true);
        ...
        >
          {currentDir ? (
            <>
              <div className="border-b border-gray-200 bg-gray-50 px-4 py-2 text-xs font-medium text-gray-500 flex">
        ...
        {previewFile && (
          <aside className="w-80 bg-white border border-gray-200 p-4 rounded-lg flex flex-col">
            ...
            <div className="flex-1 flex flex-col">
              ...
              <div className="flex-1 border rounded-lg overflow-hidden flex items-center justify-center bg-gray-50">
                {previewUrl ? (
