...
export default function BudgetPage() {
  ...
  const addExpense = () => {
    const newExpense = { category: "", description: "", date: "", amount: 0 };
    const updatedExpenses = [...expenses, newExpense];
    setExpenses(updatedExpenses);
    saveSingleField("expenses", updatedExpenses);
  };

  // ✅ Now using saveSingleField instead of a separate endpoint
  const updateExpense = async (index: number, field: string, value: string | number) => {
            ...
            <tbody className="text-gray-800">
              {expenses.map((exp, i) => (
                <tr key={i} className="border-t hover:bg-gray-50 transition">
                  <td className="p-2">
                    <InlineEditableCell value={exp.category} onSave={(val) => updateExpense(i, "category", val)} />
                  </td>
                  <td className="p-2">
                    <InlineEditableCell value={exp.description} onSave={(val) => updateExpense(i, "description", val)} />
                  </td>
                  <td className="p-2">
                    <InlineEditableCell type="date" value={exp.date} onSave={(val) => updateExpense(i, "date", val)} />
                  </td>
                  <td className="p-2">
                    <InlineEditableCell type="number" value={String(exp.amount)} onSave={(val) => updateExpense(i, "amount", Number(val))} />
                  </td>
                </tr>
              ))}
            </tbody>
...
// === Reusable components ===
function EditableField({ label, value, onSave, type = "text" }: { label: string; value: string; onSave: (val: string) => Promise<void>; type?: string }) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-md p-5 w-full md:w-1/3" onDoubleClick={() => !isEditing && setIsEditing(true)}>
      ...
      {isEditing ? (
        ...
        <p className="text-base font-medium text-gray-900 cursor-pointer">{value || <span className="text-gray-400 italic">Double-click to edit</span>}</p>
      )}
    </div>
  );
}

function InlineEditableCell({ value, onSave, type = "text" }: { value: string; onSave: (val: string) => Promise<void> | void; type?: string }) {
  const [isEditing, setIsEditing] = useState(false);
  const [localValue, setLocalValue] = useState(value);
  ...
  return isEditing ? (
    ...
    <p onDoubleClick={() => setIsEditing(true)} className="cursor-pointer">{value || <span className="text-gray-400 italic">Double-click to edit</span>}</p>
  );
}
